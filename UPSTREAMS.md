...
| context-mode | [mksglu/context-mode](https://github.com/mksglu/context-mode) | Elastic-2.0 | 1.0.169 (`6f0cc68`) | Runs big commands, files and pages in a sandbox and returns only the derived answer, so raw bytes stay out of the context window. | Its session-start injection is capped to fit Claude Code's 10,000-character hook limit (src/cap-context-mode.ts); above that the model only receives a 2 KB preview and loses the routing rules. |
| context7 | [upstash/context7](https://github.com/upstash/context7) | **none declared** | 1.0.2 (`c324828`) | Current library and framework documentation, instead of the model answering from memory. | — |
| rtk | [rtk-ai/rtk](https://github.com/rtk-ai/rtk) | **none declared** | 0.10.0 | Compresses shell output (git, tests, builds) before it reaches the model. Hooked on every Bash and PowerShell call. | — |
