...
| jev-browser | [jkudish/jev-browser](https://github.com/jkudish/jev-browser) | MIT | 0.4.1 (`f2b13a0`) | Multi-step browser tasks in headless Chromium, with Jev choosing each action: a few cents of Jev instead of Claude reading every page. Needs TYPESAFE_API_KEY. | Setup registers its MCP server for all projects (user scope). |
| canny | [qkal/canny](https://github.com/qkal/canny) | MIT | 0.3.0 (`f2c5e53`) | Hooks that block "done" until a real check (tests, typecheck, lint) has passed since the last edit, so Claude does not stop on unverified work. | Setup follows its install: clone to ~/.canny/src, then `init --global --claude` writes its hooks into ~/.claude/settings.json. |
| agent-desktop | [lahfir/agent-desktop](https://github.com/lahfir/agent-desktop) | Apache-2.0 | 0.9.4 (`a4a695f`) | Drives native macOS apps through the accessibility tree: compact element refs instead of screenshots. macOS only. | Setup installs the CLI from npm and its agent-desktop and jev-desktop skills into ~/.claude/skills. |
